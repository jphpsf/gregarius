...
class Snoopy
{
	...
	function Snoopy() {
			...
			/*
			if (function_exists('getConfig') && ($tmp = getConfig('rss.output.cachedir')) != null) {
					if (substr($tmp, -1, 1) != "/") {
						$tmp .= "/";
					}
					$this -> tmpdir = $tmp;
	...
	function fetch($URI)
	{
		...
		if($URI_PARTS["scheme"] == "https")
		{
			...
			if($use_curl || !function_exists('file_get_contents') || !extension_loaded('openssl'))
			{
				...
				// MBI: is_executable would break on some installs
				if(!$this->curl_path || (!file_exists($this->curl_path))) {					
...
\*======================================================================*/
	...
	function _httprequest($url,$fp,$URI,$http_method,$content_type="",$body="")
	{
		...
		if(!empty($this->user) || !empty($this->pass))	
			$headers .= "Authorization: BASIC ".base64_encode(urldecode($this->user).":".urldecode($this->pass))."\r\n";
...
\*======================================================================*/
	...
	function _curlrequest($url,$URI,$http_method,$content_type="",$body="")
	{
		...
		//$cmdline_params .= " -k";
		if (defined ('RSS_SNOOPY_EXTRA_PARAMS')) {
			$cmdline_params .= (" " . RSS_SNOOPY_EXTRA_PARAMS ." ");
		}
		$full_cmdline = $this->curl_path
			." -D \"".$this -> tmpdir
			."$headerfile\""
			.escapeshellcmd($cmdline_params)." "
			.escapeshellcmd($URI);
		//die($full_cmdline);
		exec($full_cmdline,$results,$return);
		...
		$result_headers = file($this -> tmpdir . $headerfile);
