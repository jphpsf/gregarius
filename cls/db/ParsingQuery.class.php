...
* @version $Id: ParsingQuery.class.php,v 1.32 2004/11/21 16:17:31 freddy78 Exp $ $Revision: 1.32 $
*/
...
class ParsingQuery {
	...
	*/
	function tabletoSQLite($query){
		...
		$finaleQuery = 'CREATE TABLE '.$this->brackets($tableName).' ('."\n";
		foreach($defineElement as $elem) {
			$column[] = $this->brackets($elem['name']).' '.$elem['type'].((isset($elem['null']) && !$elem['null'])? ' NOT NULL ' : ' ' ).$elem['sup'];
		}
		$finaleQuery .= "\t".implode(",\n\t", $column)."\n);";
		$tabQ[] = $finaleQuery;
		if(isset($tabIndex) && is_array($tabIndex)){
			foreach($tabIndex as $ind){
				...
				if (is_array($ind['champ'])) {
				    foreach ($ind['champ'] as $key=>$champ) $ind['champ'][$key] = $this->brackets($champ);
					$columns = implode(',', $ind['champ']);
				} else
				    $columns = $this->brackets($ind['champ']);				 
				$query .= ' INDEX '. str_replace(' ','_',$tableName.'_'.$ind['name']).' ON '.$this->brackets($tableName).' ('.$columns.');';
				$tabQ[] = $query;
			}
		}
		return $tabQ;
	...
	*/
	...
	/**
	*  Add brackets when spaces in objects, remove existing ones for concatenations (function to set global ?)
	*
	* @access public
	*/
	function brackets($object){
		//ALERTI: added trim
		$object = trim(ereg_replace('\[|\]','',$object));
		if (strstr($object,' ')) {
			$object = "[$object]";
		}
		return $object;
	...
	function noLimit($query){
		if(eregi('LIMIT[[:space:]]', $query)){
			...
			if(isset($limitRes[1])) {
				...
				$out['page'] = ($startRecLimit / $GLOBALS['browseNbRecordPerPage']) +1;
